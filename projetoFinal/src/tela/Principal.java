...
import Entidades.Aldeao;
import Entidades.Fazenda;
import Entidades.MinaDeOuro;
import Entidades.Prefeitura;
import Entidades.Templo;
...
public class Principal extends JFrame {
    ...
    Prefeitura prefeitura = new Prefeitura(this);
    ...
    public Principal() {
        ...
        this.clienteTCP = new ClienteTCP(this, this.prefeitura);
        initialize();
    ...
    public JComboBox<String> getCbCivilizacoes() {
        return cbCivilizacoes;
    }
    ...
    public void mostrarMensagem(String titulo, String mensagem) {
        JOptionPane meuJOPane = new JOptionPane(mensagem);
        final JDialog dialog = meuJOPane.createDialog(null, titulo);//aqui uso um JDialog para manipular                                                                 //meu JOptionPane
    ...
    //trabalhando...
    public void removerAldeao(String numero, String acao) {
        String[] linha = {numero, acao};
        for (int i = 0; i < this.tmAldeoes.getRowCount(); i++) {
            if (Integer.parseInt(numero) - 1 == i) {
                this.tmAldeoes.removeRow(i);
            }
        }
    }

    public void limparAldeoes(int i) {
        this.tmAldeoes.setRowCount(i);
    }
    ...
    //trabalhando...
    public void removerFazenda(String numero, String acao) {
    }

    public void limparFazendas(int i) {
        this.tmFazendas.setNumRows(i);
    }
    ...
    public void limparMinasOuro(int i) {
        this.tmMinasOuro.setNumRows(i);
    }
    ...
    public void desabilitarMaravilha() {
        this.pnMaravilha.setEnabled(false);
        this.lblMaravilha.setEnabled(false);
        this.pbMaravilha.setEnabled(false);
    }
    ...
    //*** Inicio ************************************************************
    private void comandoCriarJogo(String nome, String civilizacao) throws InterruptedException {
        boolean retorno;
        if (nome.length() < 2) {
        ...
        if (retorno) {
            this.clienteTCP.criarJogo(nome, civilizacao);
    ...
    private void comandoConectar(String nome, String civilizacao, String ipServidor) {
        ...
        retorno = this.clienteTCP.conectar(nome, civilizacao, ipServidor); // retorno da cria��o do jogo
        if (retorno) {
            ...
            this.mostrarMensagemErro("Erro", "Conexão não efetuada!");
        }
    }

    private void comandoDesconectar() {
        //this.clienteTCP.enviarMensagemDesconexao();
    ...
    public void comandoAldeaoParar(int aldeao) {
        if (aldeao == -1) {
            ...
            this.prefeitura.getAldeoes().get(aldeao).setAcao(AcaoAldeao.PARADO);
            mostrarAldeao(aldeao + 1, prefeitura.getAldeoes().get(aldeao).getAcao().name());
            System.out.println("comandoAldeaoParar(aldeao);");
        }
    }

    public void comandoAldeaoConstruir(int aldeao, String qual) {
        //Fazer com que a Thread atualizaConstruções só execute depois do aldeao, usando lock;
        if (aldeao == -1) {
            mostrarMensagemErro("Erro", "Escolha um aldeão");
            return;
        }
        ...
        switch (qual) {
            case "Fazenda":
                prefeitura.getAldeoes().get(aldeao).setAcao(AcaoAldeao.CONSTRUINDO_FAZENDA);
                prefeitura.getAldeoes().get(aldeao).agir();

                break;
            case "Mina de ouro":
                prefeitura.getAldeoes().get(aldeao).setAcao(AcaoAldeao.CONSTRUINDO_MINA);
                prefeitura.getAldeoes().get(aldeao).agir();

                break;
            case "Templo":
                if (this.prefeitura.getTemplo() != null) {
                    mostrarMensagemErro("Erro", "Já há um templo");
                    return;
                }
                prefeitura.getAldeoes().get(aldeao).setAcao(AcaoAldeao.CONSTRUINDO_TEMPLO);
                prefeitura.getAldeoes().get(aldeao).agir();
                break;
            case "Maravilha":
                if (this.prefeitura.getTemplo() != null) {
                    mostrarMensagemErro("Erro", "Já há uma Maravilha");
                    return;
                }
                this.prefeitura.getAldeoes().get(aldeao).setMaravilha(this.prefeitura.getMaravilha());
                prefeitura.getAldeoes().get(aldeao).setAcao(AcaoAldeao.CONSTRUINDO_MARAVILHA);
                prefeitura.getAldeoes().get(aldeao).agir();
                break;
        }
    }
    ...
    public void comandoAldeaoCultivar(int aldeao, int numeroFazenda) {
        ...
        if (prefeitura.getAldeoes().get(aldeao).getAcao() == AcaoAldeao.CULTIVANDO) {
            mostrarMensagemErro("Erro", "Aldeão já está cultivando nesta fazenda!");
            return;
        }
        System.out.println("comandoAldeaoCultivar(aldeao, numeroFazenda);");
        prefeitura.getAldeoes().get(aldeao).setFazenda(prefeitura.getFazendas().get(numeroFazenda));
        prefeitura.getAldeoes().get(aldeao).setAcao(AcaoAldeao.CULTIVANDO);
        prefeitura.getAldeoes().get(aldeao).agir();
    }
    ...
    public void comandoAldeaoMinerar(int aldeao, int numeroMinaOuro) {
        if (aldeao == -1) {
            mostrarMensagemErro("Erro", "Escolha um aldeão");
        } else if (prefeitura.getAldeoes().get(aldeao).getAcao() == AcaoAldeao.MINERANDO) {
            mostrarMensagemErro("Erro", "Aldeão já está minerando nesta mina de ouro!");
            return;
        } else {
            System.out.println("comandoAldeaoMinerar(aldeao, numeroMinaOuro);");
            prefeitura.getAldeoes().get(aldeao).setMina(prefeitura.getMinasDeOuro().get(numeroMinaOuro));
            prefeitura.getAldeoes().get(aldeao).setAcao(AcaoAldeao.MINERANDO);
            prefeitura.getAldeoes().get(aldeao).agir();
        }
    }
    ...
    public void comandoAldeaoOrar(int aldeao) {
        ...
        if (this.prefeitura.getTemplo() == null) {
            mostrarMensagemErro("Erro", "Não há um templo");
            return;
        }
        System.out.println("comandoAldeaoOrar(aldeao);");
        prefeitura.getAldeoes().get(aldeao).setAcao(AcaoAldeao.ORANDO);
        prefeitura.getAldeoes().get(aldeao).setTemplo(this.prefeitura.getTemplo());
        prefeitura.getAldeoes().get(aldeao).agir();
    ...
    public void comandoAldeaoSacrificar(int aldeao) {
        if (aldeao == -1) {
            ...
            this.prefeitura.getTemplo().setSacrificado(this.prefeitura.getAldeoes().get(aldeao));
            this.prefeitura.getTemplo().setAcao(AcaoTemplo.SACRIFICANDO);
            this.prefeitura.getTemplo().agir();
        }
    }

    public void comandoPrefeituraCriarAldeao() {
        if (this.prefeitura.getAcao() != AcaoPrefeitura.PARADA) {
            mostrarMensagemErro("Erro", "A prefeitura já está fazendo algo");
            return;
        }
        System.out.println("comandoPrefeituraCriarAldeao();" + this.prefeitura.getAldeoes().size());
        this.prefeitura.setAcao(AcaoPrefeitura.CRIANDO_ALDEAO);
        this.prefeitura.agir();
    }
    ...
    public void comandoPrefeituraEvoluir(String strEvolucao) {
        ...
        switch (strEvolucao) {
            case "Evolução de aldeão":
                this.prefeitura.setAcao(AcaoPrefeitura.EVOLUINDO_ALDEAO);
                this.prefeitura.agir();
                break;
            case "Evolução de fazenda":
                this.prefeitura.setAcao(AcaoPrefeitura.EVOLUINDO_FAZENDA);
                this.prefeitura.agir();
                break;
            case "Evolução de mina de ouro":
                this.prefeitura.setAcao(AcaoPrefeitura.EVOLUINDO_MINA);
                this.prefeitura.agir();
                break;
        }
    }
    ...
    public void comandoTemploEvoluir(String strEvolucao) {
        if (this.prefeitura.getTemplo() == null) {
            this.mostrarMensagemErro("Erro", "Não há um templo!");
            return;
        }
        System.out.println("comandoTemploEvoluir(strEvolucao);");
        this.prefeitura.getTemplo().setAcao(AcaoTemplo.EVOLUINDO);
        switch (strEvolucao) {
            case "Nuvem de gafanhotos":
                this.prefeitura.getTemplo().setPoder(Poder.NUVEM_GAFANHOTOS);
                break;
            case "Morte dos primogênitos":
                this.prefeitura.getTemplo().setPoder(Poder.MORTE_PRIMOGENITOS);
                break;
            case "Chuva de pedras":
                this.prefeitura.getTemplo().setPoder(Poder.CHUVA_PEDRAS);
                break;
            case "Proteção contra nuvem de gafanhotos":
                this.prefeitura.getTemplo().setPoder(Poder.PROTECAO_NUVEM_GAFANHOTOS);
                break;
            case "Proteção contra morte dos primogênitos":
                this.prefeitura.getTemplo().setPoder(Poder.PROTECAO_MORTE_PRIMOGENITOS);
                break;
            case "Proteção contra chuva de pedras":
                this.prefeitura.getTemplo().setPoder(Poder.PROTECAO_CHUVA_PEDRAS);
                break;
        }
        this.prefeitura.getTemplo().agir();
    }
    ...
    public void comandoTemploLancar(String strPraga, String strInimigo) {
        ...
        this.clienteTCP.enviarAtaque(strPraga, strInimigo);
